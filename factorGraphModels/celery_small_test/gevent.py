...
class Timer(_timer.Timer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.___Greenlet = _Greenlet
        self.___GreenletExit = GreenletExit
        super(Timer, self).____init__(*args, **kwargs)
        self.___queue = set()

    def _enter(self, eta, priority, entry):
        secs = max(eta - time(), 0)
        g = self.__Greenlet.spawn_later(secs, entry)
        self.__queue.add(g)
        g.link(self.__entry_exit)
        g.entry = entry
        g.eta = eta
        g.priority = priority
        g.cancelled = False
